%% Read the Benchmark
load("BenchmarksInput.mat","Xs_S2SC")
Xs_input = Xs_S2SC;
...
ds_curvature = zeros([N_curve-1,1]); % d_kappa
errors_contour = zeros([N_curve-1,1]); % d_infty, i.e., contour error between the input Xs
rho1s = zeros([N_curve-1,1]); % rho_1
rhos_curvature = zeros([N_curve-1,1]); % rho_kappa

for i = 1:N_curve-1
    errors_contour(i) = contour_error(Xs{i}(:,1:2)',Xs{i+1}(:,1:2)');
    ds_curvature(i) = d_V(Xs_output{i}(:,1:2)',abs(kappa_output{i}'),Xs_output{i+1}(:,1:2)',abs(kappa_output{i+1}'));
    rho1s(i) = rho1(Xs{i}',Xs0');
    rhos_curvature(i) = rho_V(Xs_output{i}(:,1:2)',abs(kappa_output{i}'),Xs_output{i+1}(:,1:2)',abs(kappa_output{i+1}'));
end
%% Results
