%% Read the Benchmark
load("BenchmarksInput.mat","Xs_CS")
Xs_input = Xs_CS;
...
for i = 1:N_curve-1
    errors_contour(i) = contour_error(Xs{i}',Xs{i+1}');
    ds_curvature(i) = d_V(Xs_output{i}',abs(kappa_output{i}'),Xs_output{i+1}',abs(kappa_output{i+1}'));
    rho1s(i) = rho1(Xs{i}',Xs0');
    rhos_curvature(i) = rho_V(Xs_output{i}',abs(kappa_output{i}'),Xs_output{i+1}',abs(kappa_output{i+1}'));
end
%% Results
